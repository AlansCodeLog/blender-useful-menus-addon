...
#get ui scale (not dynamic, script needs to be rerun if changed)
...
class CustomDrawOperator(bpy.types.Operator):
   ...
   col_num = math.ceil(math.sqrt(len(brush_names)/scale)) #total number of brushes divided by scale (so real pixel width and height are the same, then sqrt to keep popup relatively square
   row_num = math.ceil(len(brush_names)/col_num)
   ...
   def invoke(self, context, event):
      ...
      return wm.invoke_popup(self, width=self.col_num*button_width*ui_scale) 
